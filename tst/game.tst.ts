...
describe("handle roll action", () => {
  ...
  it("handleAction(<roll action>, 1) returns null, requester incorrect", () => {
    const e = (<Game>game).handleAction(
      new Action(ActionType.Roll, 1, { dice: [1, 2] })
    );
...
describe("roll & endturn actions", () => {
  ...
  it("roll, endturn, roll, endturn", () => {
    game.handleAction(new Action(ActionType.Roll, 0, { dice: [1, 2] }));
    assert.strictEqual(game.turnState, TurnState.Postroll);
    game.handleAction(new Action(ActionType.EndTurn, 0));
    assert.strictEqual(game.turnState, TurnState.Preroll);
    assert.strictEqual(game.turn, 1);

    game.handleAction(new Action(ActionType.Roll, 1, { dice: [1, 2] }));
    assert.strictEqual(game.turnState, TurnState.Postroll);
...
describe("setup phases", () => {
  it("works", () => {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      act = g.handleAction(
        new Action(ActionType.BuildSettlement, i, { node: 2 * i })
      );
      ...
      act = g.handleAction(
        new Action(ActionType.BuildRoad, i, { node0: 2 * i, node1: 2 * i + 8 })
      );
    ...
    for (let i = 3; i >= 0; i--) {
      ...
      act = g.handleAction(
        new Action(ActionType.BuildSettlement, i, { node: 7 + 2 * i })
      );
      ...
      act = g.handleAction(
        new Action(ActionType.BuildRoad, i, {
          node0: 7 + 2 * i,
          node1: 17 + 2 * i,
        })
      );
...
describe("setup space 20", () => {
  it("huh", () => {
    const g: Game = new Game();
    const a = g.handleAction(
      new Action(ActionType.BuildSettlement, 0, { node: 19 })
    );
